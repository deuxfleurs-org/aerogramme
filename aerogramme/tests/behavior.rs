...
fn rfc3501_imap4rev1_recent() {
    ...
    common::aerogramme_provider_daemon_dev_multisession(|server| {
        ...
        let status_res =
            status(imap_socket, Mailbox::Inbox, StatusKind::Recent).context("status INBOX")?;
        ...
        let select_res =
            select(imap_socket, Mailbox::Inbox, SelectMod::None).context("examine INBOX")?;
        assert!(select_res.contains("2 RECENT"));
        {
            ...
            let status_res =
                status(imap_socket2, Mailbox::Inbox, StatusKind::Recent).context("status INBOX")?;
    ...
    common::aerogramme_provider_daemon_dev_multisession(|server| {
        ...
        assert!(res.contains("\\Recent"));
        let res =
            status(imap_socket3, Mailbox::Inbox, StatusKind::Recent).context("status INBOX")?;
...
fn rfc3501_imap4rev1_uidvalidity() {
    ...
    common::aerogramme_provider_daemon_dev(|imap_socket, _lmtp_socket, _dav_socket| {
        ...
        let res =
            status(imap_socket, Mailbox::Archive, StatusKind::UidValidity).context("status")?;
