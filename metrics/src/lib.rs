...
lazy_static::lazy_static! {
    pub static ref NB_CREATED_SESSIONS: IntCounter = IntCounter::with_opts(
        Opts::new("imap_nb_created_sessions", "Number of created IMAP sessions since server run")
    ).unwrap();

    pub static ref NB_CURRENT_SESSIONS: IntGauge = IntGauge::with_opts(
...
impl MetricServer {
  pub fn new(config: PrometheusEndpointConfig) -> Result<Self> {
    ...
    registry.register(Box::new(NB_CREATED_SESSIONS.clone()))?;
    registry.register(Box::new(NB_CURRENT_SESSIONS.clone()))?;
