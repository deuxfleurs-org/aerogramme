...
pub struct MailboxView {
    ...
    ///
    /// `recent_from` is loaded once from the index when the `MailboxView` is
    /// created and not updated afterwards: it represents the session-local view
    /// of what is "recent", defined as any mail added after the beginning of
    /// the session.
    ///
...
impl MailboxView {
    ...
    pub async fn new(mailbox: Mailbox, is_condstore: bool) -> Result<Self> {
        ...
        let recent_from = known_state.recent_from;
        Ok(Self {
            mailbox,
            known_state,
            is_condstore,
    ...
    pub async fn bump_recent(&mut self) -> Result<()> {
        self.mailbox.bump_recent().await
    }
    ...
    pub async fn store<'a>(
        ...
        let conflict_id_or_uid = match is_uid_store {
            ...
            false => in_conflict.into_iter().map(|midx| midx.seqid).collect(),
        };
    ...
    pub async fn copy(
        ...
        let state = self.mailbox.current_uid_index();
        let mails = state.fetch(
            sequence_set,
            self.recent_from,
            FetchBy::from_bool(*is_uid_copy),
        );
    ...
    pub async fn search<'a>(
        ...
        // 2. Get the selection
        let selection = state.fetch(
            &seq_set,
            self.recent_from,
            FetchBy::from_bool(seq_type.is_uid()),
        );
